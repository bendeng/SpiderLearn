...
#https://blog.mimvp.com/article/8257.html

try:
    # 1.链接mongod的服务
    mongo_py = pymongo.MongoClient("%s:%d"%(host, port))
    mongo_py[db_name].authenticate(user_name, user_pwd)
    ...
    # 修改数据
    #collection.update_one({"age": 20}, {"$set": {"name": "小王"}})
